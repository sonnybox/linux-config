...
local osc_styles = {
	s24Button = "{\\blur0\\bord0\\1c&HFFEFED\\3c&HFFFFFF\\fs24\\fnmpv-osd-symbols}",
	s16Button = "{\\blur0\\bord0\\1c&H8E807D\\3c&HFFFFFF\\fs16\\fn" .. user_opts.font .. "}",
	cyButton = "{\\blur0\\bord0\\1c&HFFEFED\\3c&HFFFFFF\\fs20\\fnmpv-osd-symbols}",
	...
	fgSeekbar = "{\\blur0\\bord0\\1c&cccccc\\3c&HFFFFFF\\fs13}",
				...
				for _, marker in pairs(markers) do
					if (marker > element.slider.min.value) and (marker < element.slider.max.value) then
						...
						if slider_lo.gap > 1 then -- draw triangles
							...
							--top
							if slider_lo.nibbles_top then
								static_ass:rect_cw(s - 1, slider_lo.border, s + 1,
...
function render_elements(master_ass)
	for n = 1, #elements do
		...
		if element.type == "slider" then
			...
			if slider_lo.stype ~= "bar" then
				foH = elem_geo.h * user_opts.seekbarhandlesize / 2
			...
			if pos then
				...
				if slider_lo.stype ~= "bar" then
					...
					ass_draw_rr_h_cw(elem_ass, xp - rh, foH - rh, xp + rh, foH + rh, rh,
						slider_lo.stype == "diamond")
					ass_draw_rr_h_cw(elem_ass, s_min, foH - r, xp, foH + r, r,
						slider_lo.stype == "diamond")
				else
					local h = 0
					...
					if seekRanges and not user_opts.seekrangeseparate and slider_lo.rtype ~= "inverted" then
						...
						for _, range in pairs(seekRanges) do
							if range["start"] < pos then
								local pstart = get_slider_ele_pos_for(element,
									range["start"])
								local pend = xp

								if pos > range["end"] then
									pend = get_slider_ele_pos_for(element,
										range["end"])
								end
								elem_ass:rect_ccw(
									pstart,
			...
			if not (maxchars == nil) and (charcount > maxchars) then
				...
				if charcount > limit then
					while charcount > limit do
						...
						charcount = (buttontext:len() + select(2, buttontext:gsub("[^\128-\193]", "")) * 2) /
						3
					end
...
function window_controls(topbar)
	...
	lo.geometry = { x = titlebox_left + left_pad, y = wc_geo.y - 3 - 3, an = 1, w = titlebox_w, h = wc_geo.h }
...
layouts["default"] = function()
	...
	local posY = math.floor(get_align(user_opts.valign, osc_param.playresy, osc_geo.h, -50))
...
function validate_user_opts()
	...
	if
	    user_opts.seekrangestyle ~= "bar"
	    and user_opts.seekrangestyle ~= "line"
	    and user_opts.seekrangestyle ~= "slider"
	    and user_opts.seekrangestyle ~= "inverted"
	    and user_opts.seekrangestyle ~= "none"
	then
...
function osc_init()
	...
	ne.content = function()
		...
		return "Cache: " ..
		(min > 0 and string.format("%sm%02.0fs", min, sec) or string.format("%3.0fs", dmx_cache))
	end
	...
	if not (state.anitype == nil) then
		...
		if now < state.anistart + (user_opts.fadeduration / 1000) then
			if state.anitype == "in" then --fade in
				osc_visible(true)
				state.animation =
				    scale_value(state.anistart, (state.anistart + (user_opts.fadeduration / 1000)), 255,
					    0, now)
			elseif state.anitype == "out" then --fade out
				state.animation =
				    scale_value(state.anistart, (state.anistart + (user_opts.fadeduration / 1000)), 0,
					    255, now)
			end
		else
...
function tick()
	...
	if state.idle then
		...
		local line_prefix = ("{\\rDefault\\an7\\1a&H00&\\bord0\\shad0\\pos(%f,%f)\\fscx300\\fscy300}"):format(
			icon_x,
			icon_y
		)
